...
export default function SearchCard({id,title,priceAfterDiscount,price,discountPercentage,stock,images,rating,description,warranty}) {
  return (
    <Link to={`/product/${id}/${title}`} className="flex flex-col gap-4 w-[80%]">
      {/* Single Card */}
      <div className="flex flex-col md:flex-row border rounded-lg p-4 gap-4 shadow-sm">
        ...
        <div className="flex flex-col items-start md:items-end justify-between">
          <div>
            ...
            <div className="text-green-600 text-sm font-semibold">{discountPercentage} off</div>
          </div>
        </div>
      </div>
    </Link>
